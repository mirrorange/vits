...
class vits_inference:
    ...
    def __init__(self,model_name):
        self.text_utils = __import__("model.{}.text".format(model_name),fromlist=["text"])
        self.load_model(os.path.join("model",model_name,"config.json"),os.path.join("model",model_name,"model.pth"))

    def get_text(self,text, hps):
        text_norm = self.text_utils.text_to_sequence(text, hps.data.text_cleaners)
...
if __name__ == '__main__':
    ...
    while True:
        ...
        output_file = os.path.join(output_dir,"{}_{}.wav".format(model_name,target_text.replace(" ","_")))
